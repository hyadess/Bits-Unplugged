...
import ProblemSetTopicCard from "../Components/ProblemSetTopicCard";
import { async } from "q";
...
const ProblemSet = () => {
  ...
  const deleteAProblem = async(problem_id)=>{
    ...
    if(res.success)
    {
      setProblemList(problemList.filter(
        (problemList) =>
      ...
      {!loading && (
        <div className="flex flex-row flex-wrap items-center justify-between items-center pb-4 px-4 mx-auto max-w-screen-2xl xl:gap-16 md:grid md:grid-cols-4 sm:py-6 lg:px-6">
          {problemList.map((prob, index) => (
            <ProblemSetCard id={prob.problem_id} name={prob.title} deleteAction={deleteAProblem} is_live={prob.is_live}/>
          ))}
        </div>
      )}
