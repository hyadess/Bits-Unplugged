...
const LeaderBoardCard = ({ id, contest_id, name, image, points, path }) => {
  ...
  const getAll = async () => {
    ...
    const res = await contestApi.totalProblemCount(contest_id);
    if (res.success) {
      setTotal(res.data[0].totalProblems);
    }
    const res2 = await contestApi.totalProblemSolved(contest_id, id);
    if (res2.success) {
      setSolved(res2.data[0].totalSolved);
    }
  };
        ...
        <div className="flex flex-row border rounded-lg shadow-md bu-card-primary h-full w-full p-5">
          <div className="flex flex-col gap-1 justify-between w-[70%]">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="bu-text-subtitle">{points} points</div>
            </div>
